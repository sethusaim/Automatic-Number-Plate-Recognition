...
class PlateHistory:
        ...
        for (plateText, chBoxes, plateBox, plateScore) in zip(
            ...
            if len(plateText) != 0:
                if plateText in self.rollingPlateDict:
                    self.rollingPlateDict[plateText].append(
                        [
                            plateText,
                            chBoxes,
                            plateBox,
                    ...
                    self.rollingPlateDict[plateText] = [
                        [
                            plateText,
                            chBoxes,
                            plateBox,
    ...
    def writeToFile(
        self, plateDict, destFolderRootName, imageWidth, imageHeight, imageDepth
    ):
        plateAnn = PlateAnn()
        ...
        for plateTextKey in plateDictForFullLog.keys():
            (
                ...
                numberOfPlates,
                plateScore,
            ) = plateDictForFullLog[plateTextKey]
            ...
            outputFullImageFileName = "{}_{}_{}_cnt{}_score_{:.2f}".format(
                fileNamePrefix, self.fileCnt, plateText, numberOfPlates, plateScore
            )
            outputFullImagePath = "{}/{}/{}.jpg".format(
                self.output_image_path, destFolderRootName, outputFullImageFileName
            )
            outputAnnPath = "{}/{}_ann/{}.xml".format(
                self.output_image_path, destFolderRootName, outputFullImageFileName
            )
            plateAnn.writeAnnFile(
                outputAnnPath,
                outputFullImagePath,
                plateBox,
                plateText,
            ...
            if self.saveAnnotatedImage == True:
                startX, startY, endX, endY = self.scaleBB(
                    plateBox, imageWidth, imageHeight
                )
                ...
                for chBox in chBoxes:
                    startX, startY, endX, endY = self.scaleBB(
                        chBox, imageWidth, imageHeight
                    )
                    cv2.rectangle(
                        fullImage, (startX, startY), (endX, endY), (0, 255, 0), 2
                    )
            ...
            imageFileName = "{}/{}.jpg".format(
                destFolderRootName, outputFullImageFileName
            )
        ...
        for plateTextKey in plateDictForPartialLog.keys():
            (
                ...
                numberOfPlates,
                plateScore,
            ) = plateDictForPartialLog[plateTextKey]
