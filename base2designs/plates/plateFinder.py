...
class Plate_Finder:
    ...
    def processPlates(self, plates, plate_boxes, plateScores):
        ...
        for (i, (plateBox, plateScore, chScores)) in enumerate(
            ...
            if (
                averageScore < self.minScore
                or len(chScores) <= self.minChars
                or max(plateBox) >= 0.998
                or min(plateBox) <= 0.002
            ):
                mask[i] = False
    ...
    def findCharsOnly(
        ...
        for (charBox, score, label) in zip(boxes, scores, labels):
            ...
            (pbStartY, pbStartX, pbEndY, pbEndX) = (
                int(plateBox[0] * fullImageHeight),
            ...
            charBox = (
                (charBox[0] * pbHeight + pbStartY) / fullImageHeight,
